...
const parseXml = (xml, options) =>
  Future.node(done => parseString(xml, options, done))

const processStringValues = R.when(
  R.is(String),
  R.pipe(R.trim, trimTabsAndNewlines)
)
  ...
  return parseXml(rawData, {
    ...
    .chain(
      R.ifElse(
        ...
        Future.of,
        // Else
        R.pipe(R.prop('error'), cNsApiError('API error'), Future.reject)
      )
    )
    .chain(data => {
      // TODO: test this
      const { faultcode, faultstring } = R.defaultTo(
        {},
        R.view(soapFault, data)
      )
      if (faultcode != null) {
